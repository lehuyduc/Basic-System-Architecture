...
public class Launch {
	...
	public static void main(String[] args) {
		try {
			int nbnodes = args.length; 
			Vector<Integer> ports = new Vector<>();
			for (int i=0; i<nbnodes; i++) ports.add(Integer.parseInt(args[i]));
			...
			for (int i=0; i<nbnodes; i++) {
				// Naming.lookup require external rmiregistry console, but we don't want that 
				//d[i] = (Daemon)Naming.lookup("//localhost/daemon" + i);
				
				// Daemon port + port_offset = port of registry
				Registry registry = LocateRegistry.getRegistry(ports.elementAt(i) + port_offset);
				d[i] = (Daemon)registry.lookup("//localhost/daemon" + i); 
